...
def a_star(board) -> list[tuple]:  # path最后形式(5, 6, -1, 1)，这里得出来的path
    ...
    final_direct_list = []  # 对应最后输出需要的direction
    ...
    while len(unused_list) > 0:  # 只要还有起始点，说明还有可能有更优解
        ...
        if cur_node.position != parent_node.position:  # 两者的node的位置不同
            ...
            update_board(board, parent_node.position, direction)
        ...
        "test!!!!!!!!!!!!!"
        all_red_token = get_red_position(board)
        for red_token in all_red_token: #看棋盘spread之后得到了几个棋子，得到两个棋子就要loop两次
            #cur_node.position = red_token!! #所有的棋子都要创建成node
