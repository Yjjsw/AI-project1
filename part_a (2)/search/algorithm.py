...
def a_star(board) -> list[tuple]:  # path最后形式(5, 6, -1, 1)，这里得出来的path
    ...
    change_token_position = [] # 哪些位置的token将改变
    ...
    while len(unused_list) > 0:  # 只要还有起始点，说明还有可能有更优解
        ...
        if cur_node.position != parent_node.position:  # 两者的node的位置不同, 这里是为了跳过第一次的情况
            ...
            update_board(board, parent_node.position, direction) #更新棋盘
